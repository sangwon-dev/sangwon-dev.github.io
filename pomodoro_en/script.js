...
function startTimer() {
  ...
  timer = setInterval(() => {
    ...
    if (seconds < 0) {
      ...
      if (minutes < 0) {
        ...
        let blinkInterval = setInterval(() => {
          document.title =
            document.title === '⏰ TIMEOUT!' ? ' ' : '⏰ TIMEOUT!';
          setFavicon(
        ...
        setTimeout(() => {
          ...
          document.title = 'Time Management for Concentration - POMODORO';
    ...
    document.title = `${minutesDisplay.textContent}:${secondsDisplay.textContent} - POMODORO (Time Management for Concentration)`;
  }, 1000);
}
